...
func (rf *RTPForwarder) ForwardVideoPacket(packet *rtp.Packet) {
	...
	// Preserve camera frame timestamps across fragmented RTP packets.
	packet = packet.Clone()
		...
		if client.transportMode == TransportUDP {
			...
			if client.tcpConn != nil {
				if err := rf.sendInterleavedRTP(client.tcpConn, client.videoRTPChannel, data); err != nil {
					...
					rf.firstVideoPacket = false
					core.Logger.Info().Msgf("Successfully sent first video packet to TCP client %s on channel %d",
...
func (rf *RTPForwarder) PlayClient(id string) {
	rf.mutex.Lock()
	defer rf.mutex.Unlock()
	if c := rf.clients[id]; c != nil {
		c.playing = true
	}
}
